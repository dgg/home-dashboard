export class SolarTransitCard extends HTMLElement {
	...
	#update() {
		...
		this.#updateSlot("sunrise", sunriseStr)
		this.#updateSlot("daylight", daylightStr)
		this.#updateSlot("sunset", sunsetStr)
	}

	#updateSlot(name, value) {
		let slotEl = this.querySelector(`[slot="${name}"]`)
		if (!slotEl) {
