...
- **UI** Fluent 2 Web Components via `@fluentui/web-components` (CDN)
- **Hosting** GitHub Pages, served from `/dist` on `main`
- **Data** several external APIs, fetched client-side at runtime

## Data sources
| Source | What it provides | Refresh cadence |
|---|---|---|
| `https://api.energidataservice.dk/dataset/DayAheadPrices` | DK1 spot prices (EUR/MWh) | Once daily, published ~13:05 CET - poll after that |
...
public/
	...
	styles/			# Fluent 2 token overrides, kiosk layout
	scripts/
...
data/				# examples of api responses
```
...
- Electricity prices: schedule one fetch per day, no earlier than 13:05 CET.
